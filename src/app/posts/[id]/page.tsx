...
export async function generateStaticParams() {
  ...
  const filteredPosts =
    process.env.NODE_ENV === "production" ? posts.filter((post) => post.status === "published") : posts
...
export default async function PostPage({ params }: PageProps) {
  ...
  if (!post || (process.env.NODE_ENV === "production" && post.status !== "published" && !isAdminEnabled)) {
    notFound()
  }
